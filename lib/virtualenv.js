...
function executeCommand(command) {
  ...
  exec(command, function (error, stdout) {
    if (error) {
      ...
      deferrable.resolve(stdout.trim().substr(7));
    }
  });
...
function systemInterpreterVersion() {
  return executeCommand('python --systemInterpreterVersion');
}
 ...
 * @returns {Q.promise}
 */
function executeScript(scriptPath, pythonEnv) {
  return executeCommand((pythonEnv || __dirname + '/../env/') + 'bin/python ' + scriptPath);
}
 ...
 * @returns {Q.promise}
 */
function executeBin(bin, pythonEnv) {
  return executeCommand((pythonEnv || __dirname + '/../env/') + 'bin/' + bin);
}
